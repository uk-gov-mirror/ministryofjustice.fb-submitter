class ProcessSubmissionJob < ApplicationJob
  ...
  def perform(submission_id:)
    ...
    @submission.detail_objects.to_a.each do |mail|
      body_part_content = retrieve_mail_body_parts(mail, url_resolver)
  ...
  def retrieve_mail_body_parts(mail, url_resolver)
    body_part_map = download_body_parts(mail, url_resolver)
    read_downloaded_body_parts(mail, body_part_map)
  end

  def download_body_parts(mail, url_resolver)

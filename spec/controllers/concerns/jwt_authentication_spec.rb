...
RSpec.describe 'Concerns::JWTAuthentication' do
  ...
  let(:fake_client) { OpenStruct.new(get: service_token, public_key: Base64.strict_decode64(encoded_public_key)) }
  ...
  before do
    allow(ServiceTokenService).to receive(:new).with(service_slug:, request_id:).and_return(fake_client)
